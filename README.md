...
Production requires `SECRET_KEY`, `SECURITY_PASSWORD_SALT`, and `GATEWAY_AUTH_SECRET`.
`GATEWAY_AUTH_SECRET` must match the identity helper's value. Set `FLASK_DEBUG=false`
